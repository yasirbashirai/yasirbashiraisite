...
const services = [
  ...
  { icon: "🛒", name: "E-commerce Solutions", desc: "Store design, CRO & business building that converts" },
  ...
  { icon: "🎨", name: "AI Identity Design", desc: "AI-powered visual identity that stands out" },
...
const ServicesGrid = () => (
  <Section id="services" className="py-14 md:py-16 px-4">
    <div className="max-w-7xl mx-auto">
      <div className="text-center max-w-3xl mx-auto mb-10">
        ...
        <h2 className="font-heading font-bold text-3xl md:text-5xl text-foreground mt-4 mb-3 leading-tight">
          The tools &amp; systems I master <span className="font-serif-italic gradient-text">for your growth.</span>
        </h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {services.map((s) => (
          <div
            key={s.name}
            className="relative rounded-2xl p-5 flex flex-col gap-3 bg-white border border-primary/15 shadow-card hover:shadow-card-hover hover:-translate-y-1 transition-all duration-300 grain-bg"
          >
            <div className="flex items-center gap-3 flex-wrap">
              <div className="icon-box text-xl shrink-0">{s.icon}</div>
              <span
                className="inline-flex items-center font-heading font-bold text-sm text-white px-3 py-1.5 rounded-full shadow-soft whitespace-nowrap"
                style={{
                  background:
