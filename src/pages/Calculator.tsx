...
const sliders: SliderDef[] = [
  ...
  { key: "teamSize", label: "Number of Employees / Team Members", emoji: "👥", min: 1, max: 50, step: 1, defaultVal: 3, group: "BUSINESS BASICS", helper: "Include yourself, freelancers, VAs" },
  ...
  { key: "contentHours", label: "Hours/week creating content & managing social manually", emoji: "📱", min: 0, max: 30, step: 1, defaultVal: 8, suffix: " hrs/week", group: "OPERATIONAL OVERHEAD" },
];
  ...
  return (
    <div className="mb-5">
      ...
      <input
        ...
        style={{
          background: `linear-gradient(to right, hsl(323, 100%, 44%) ${pct}%, rgba(255,255,255,0.12) ${pct}%)`,
        }}
      />
...
const Metric = ({ label, tooltip, children, color = "text-foreground" }: { label: string; tooltip: string; children: React.ReactNode; color?: string }) => (
  <Tooltip>
    <TooltipTrigger asChild>
      <div className="flex items-center justify-between py-2 border-b border-border/30 cursor-help">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-12">
            <span className="glass-pill mb-4 inline-flex">
              <Calculator className="w-4 h-4 text-primary" /> Free ROI Calculator
            </span>
            <h1 className="font-heading font-extrabold text-4xl md:text-5xl lg:text-6xl text-foreground mt-4 mb-4">
              Find Out Exactly How Much<br />
              Your Business Is <span className="gradient-text">Losing Without AI</span>
            </h1>
            <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
              Enter your real numbers below. Watch your ROI calculate in real time. No fluff. Just data.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-7 glass-card p-6 md:p-8">
              <h2 className="font-heading font-bold text-xl text-foreground mb-1">📊 Your Current Business Reality</h2>
              <p className="text-sm text-muted-foreground mb-6">Drag the sliders to match your situation</p>

              {Object.entries(groups).map(([groupName, fields]) => (
                <div key={groupName} className="mb-6">
                  <p className="text-xs font-heading font-bold uppercase tracking-widest text-primary mb-4 border-b border-primary/10 pb-2">
                    {groupName}
                  </p>
                  {fields.map((f) => (
            ...
            <div className="lg:col-span-5 flex flex-col gap-5">
              <div className="glass-card p-6 sticky top-24">
                <h2 className="font-heading font-bold text-xl text-foreground mb-1">⚡ Your AI-Powered Results</h2>
                <p className="text-sm text-muted-foreground mb-5">Updates in real time as you adjust</p>
                ...
                <div className="rounded-xl border border-destructive/20 bg-destructive/5 p-4 mb-4">
                  ...
                  <Metric label="💸 Revenue lost to manual work weekly" tooltip="(Manual hours + lead chase hours) × team size × hourly cost" color="text-destructive">
                  ...
                  <Metric label="⌛ Hours wasted per year" tooltip="(Manual + lead chase + content hours) × 52 weeks" color="text-warning">
                  ...
                  <div className="mt-3 pt-3 border-t border-destructive/20 text-center">
                    <p className="text-xs text-muted-foreground mb-1">🔥 Total Annual Revenue Loss</p>
                ...
                {/* GAINS */}
                <div className="rounded-xl border border-success/20 bg-success/5 p-4 mb-4">
                  <h3 className="text-sm font-heading font-bold text-success mb-3">✅ With Yasir's AI Systems</h3>
                  <Metric label="⏱️ Hours saved per month" tooltip="(Manual + lead chase + content) × 4 weeks × 85% automation" color="text-success">
                    <AnimNum value={results.hoursSavedPerMonth} prefix="" suffix=" hrs" />
                  </Metric>
                  <Metric label="📈 Revenue from instant AI response" tooltip="Leads × 40% capture × conversion rate × client value" color="text-success">
                  ...
                  <Metric label="👥 Team hours freed monthly" tooltip="(Manual + lead chase) × team size × 80% × 4 weeks" color="text-success">
                  ...
                  <div className="mt-3 pt-3 border-t border-success/20 text-center">
                    <p className="text-xs text-muted-foreground mb-1">🚀 Projected Additional Annual Revenue</p>
                ...
                <div className="rounded-xl border border-primary/20 bg-primary/5 p-4 mb-4">
                  ...
                  <div className="flex justify-between text-[10px] text-muted-foreground mt-1">
                    <span>Break Even</span><span>2x</span><span>5x</span><span>10x</span>
                  </div>
                </div>

                {/* TICKER */}
                <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-4 text-center mb-4">
                  <p className="text-xs text-muted-foreground mb-1">⚠️ While you read this, your business has lost:</p>
                  <p className="font-heading font-extrabold text-xl text-destructive" style={{ textShadow: `0 0 ${Math.min(ticker * 2, 30)}px rgba(255,68,68,0.6)` }}>
                    ${ticker.toFixed(2)}
                  </p>
                  <p className="text-[10px] text-muted-foreground mt-1">Based on your numbers above</p>
                </div>
                ...
                <div className="text-center">
                  ...
                  <a href="https://cal.com/yasir-bashir-bp4wob/30min" target="_blank" rel="noopener noreferrer" className="btn-gradient w-full justify-center mb-2">
                    🔥 Book My Free AI Audit <ArrowRight className="w-4 h-4 btn-icon" />
                  </a>
                  <button onClick={share} className="btn-glass w-full justify-center text-xs">
                    {copied ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
                    {copied ? "Copied!" : "Share My Results"}
                  </button>
                  <p className="text-[10px] text-muted-foreground mt-3">✅ No commitment &nbsp; ✅ 30 minutes &nbsp; ✅ 100% free</p>
                </div>
              </div>
            </div>
          </div>
